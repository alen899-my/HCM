...
//   • Compact rows — small padding, tabular numbers, horizontal row dividers
//   • Checkbox column on the LEFT (select-all for the current page)
//   • S.No column (numbering continues across pages)
//   • Scrollable body + sticky header (maxHeight prop)
//   • Action buttons injected via the `actions` render prop
//   • Skeleton loading rows, empty state, slim server-driven pagination footer
//
// Usage:
//   <DataTable
...
export interface DataTableProps<T> {
  ...
  snoColumn?: boolean;
  /** Max height of the scrollable body — "60vh" by default. */
  maxHeight?: string;
            ...
            <TableRow className="border-b border-primary bg-primary hover:bg-primary dark:bg-primary/90 dark:hover:bg-primary/90 dark:border-primary/80">
              ...
              {columns.map((col) => (
                <TableHead
                  key={col.key}
                  className={cn(
                    "border-r border-primary-foreground/20 py-1.5 text-[11px] font-semibold tracking-wide text-primary-foreground uppercase last:border-r-0",
                    col.className
                  )}
                >
                  {col.header}
              ...
              {actions && (
                <TableHead className="w-20 border-l border-primary-foreground/20 py-1.5 text-right text-[11px] font-semibold tracking-wide text-primary-foreground uppercase">
                  Actions
                </TableHead>
              )}
            </TableRow>
            ...
            {loading ? (
              Array.from({ length: 8 }).map((_, i) => (
                <TableRow key={`skeleton-${i}`} className="border-b border-border/60 dark:border-border/80">
                  {Array.from({ length: totalColumns }).map((__, j) => (
                    <TableCell
                      key={j}
                      className="border-r border-border/40 px-2 py-1.5 last:border-r-0 dark:border-border/70"
                    >
              ...
              data.map((row, idx) => {
                ...
                return (
                  ...
                  >
                    {checkboxColumn && (
                      <TableCell className="border-r border-border/40 py-1.5 pl-2 pr-0 dark:border-border/70">
                        <Checkbox
                          checked={selected}
                    ...
                    {snoColumn && (
                      <TableCell className="border-r border-border/40 py-1.5 pr-0 text-muted-foreground tabular-nums dark:border-border/70">
                        {startRow + idx}
                      </TableCell>
                    )}
                    {columns.map((col) => (
                      <TableCell
                        key={col.key}
                        className={cn(
                          "border-r border-border/40 py-1.5 last:border-r-0 dark:border-border/70",
                          col.className
                        )}
                      >
                        {col.cell(row)}
                    ...
                    {actions && (
                      <TableCell className="border-l border-border/40 py-1.5 dark:border-border/70">
