...
// features/permissions/hooks.ts
// ─────────────────────────────────────────────────────────────────────────────
// Data-fetching hooks for the Permissions module.
...
export function usePermissions(params: PermissionListParams): UsePermissionsResult {
  ...
  const fetch = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await permissionApi.list(JSON.parse(paramsKey));
      if (res.success) {
        setData(res.data ?? []);
...
export function useCreatePermission(): UseMutationResult & {
  create: (payload: PermissionPayload) => Promise<boolean>;
} {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const create = useCallback(async (payload: PermissionPayload): Promise<boolean> => {
    setLoading(true);
    setError(null);
    try {
      const res = await permissionApi.create(payload);
      if (!res.success) {
        setError(res.message || "Failed to create permission.");
        return false;
      }
      return true;
    } catch (err) {
...
export function useUpdatePermission(): UseMutationResult & {
  update: (id: string, payload: PermissionPayload) => Promise<boolean>;
} {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const update = useCallback(
    async (id: string, payload: PermissionPayload): Promise<boolean> => {
      setLoading(true);
      setError(null);
      try {
...
export function useDeletePermission(): UseMutationResult & {
  ...
  const remove = useCallback(async (id: string): Promise<boolean> => {
    ...
    setError(null);
    try {
      const res = await permissionApi.remove(id);
      if (!res.success) {
        setError(res.message || "Failed to delete permission.");
        return false;
      }
      return true;
    } catch (err) {
